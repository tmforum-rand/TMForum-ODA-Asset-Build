...
exports.createBucket = function(req, res, next) {
   ...
   *
   * bucket Bucket_Create The Bucket to be created
   * returns Bucket
   **/

  console.log('createBucket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument bucket */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createBucket', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createBucket', payload))
    .then(payload => {
    ...
    .catch( error => {
      console.log("createBucket: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteBucket = function(req, res, next) {
  ...
  console.log('deleteBucket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listBucket = function(req, res, next) {
  /**
  ...
  console.log('listBucket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listBucket: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveBucket = function(req, res, next) {
  /**
  ...
  console.log('retrieveBucket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveBucket: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
