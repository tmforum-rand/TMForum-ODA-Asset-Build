...
exports.createReserveProductStock = function(req, res, next) {
  /**
   * Creates a ReserveProductStock
   * This operation creates a ReserveProductStock entity.
   *
   * reserveProductStock ReserveProductStock_Create The ReserveProductStock to be created
   * returns ReserveProductStock
   **/

  console.log('createReserveProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument reserveProductStock */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createReserveProductStock', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createReserveProductStock', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createReserveProductStock: error=" + error);
    ...
    .catch( error => {
      console.log("createReserveProductStock: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listReserveProductStock = function(req, res, next) {
  /**
  ...
  console.log('listReserveProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listReserveProductStock: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveReserveProductStock = function(req, res, next) {
  /**
  ...
  console.log('retrieveReserveProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveReserveProductStock: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
