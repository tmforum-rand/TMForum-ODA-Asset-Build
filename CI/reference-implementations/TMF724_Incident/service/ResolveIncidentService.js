...
exports.createResolveIncident = function(req, res, next) {
  /**
   * Creates a ResolveIncident
   * This operation creates a ResolveIncident entity.
   *
   * resolveIncident ResolveIncident_Create The ResolveIncident to be created
   * returns ResolveIncident
   **/

  console.log('createResolveIncident :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument resolveIncident */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createResolveIncident', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createResolveIncident', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createResolveIncident: error=" + error);
    ...
    .catch( error => {
      console.log("createResolveIncident: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listResolveIncident = function(req, res, next) {
  /**
  ...
  console.log('listResolveIncident :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listResolveIncident: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveResolveIncident = function(req, res, next) {
  /**
   ...
   *
   * id String Identifier of the ResolveIncident
   * fields String Comma-separated properties to provide in response (optional)
   * returns ResolveIncident
   **/

  console.log('retrieveResolveIncident :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveResolveIncident: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
