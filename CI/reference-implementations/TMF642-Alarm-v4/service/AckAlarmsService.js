...
exports.createAckAlarms = function(req, res, next) {
  /**
   * Creates a AckAlarms
   * This operation creates a AckAlarms entity.
   *
   * ackAlarms AckAlarms_Create The AckAlarms to be created
   * returns AckAlarms
   **/

  console.log('createAckAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument ackAlarms */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createAckAlarms', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createAckAlarms', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createAckAlarms: error=" + error);
    ...
    .catch( error => {
      console.log("createAckAlarms: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listAckAlarms = function(req, res, next) {
  /**
  ...
  console.log('listAckAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listAckAlarms: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveAckAlarms = function(req, res, next) {
  /**
  ...
  console.log('retrieveAckAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveAckAlarms: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
