...
exports.createUnAckAlarms = function(req, res, next) {
  /**
   * Creates a UnAckAlarms
   * This operation creates a UnAckAlarms entity.
   *
   * unAckAlarms UnAckAlarms_Create The UnAckAlarms to be created
   * returns UnAckAlarms
   **/

  console.log('createUnAckAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument unAckAlarms */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createUnAckAlarms', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createUnAckAlarms', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createUnAckAlarms: error=" + error);
    ...
    .catch( error => {
      console.log("createUnAckAlarms: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listUnAckAlarms = function(req, res, next) {
  /**
  ...
  console.log('listUnAckAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listUnAckAlarms: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveUnAckAlarms = function(req, res, next) {
  /**
  ...
  console.log('retrieveUnAckAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveUnAckAlarms: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
