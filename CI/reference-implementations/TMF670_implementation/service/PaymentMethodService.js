...
exports.createPaymentMethod = function(req, res, next) {
  /**
   * Creates a PaymentMethod
   * This operation creates a PaymentMethod entity.
   *
   * paymentMethod PaymentMethod_Create The PaymentMethod to be created
   * returns PaymentMethod
   **/

  console.log('createPaymentMethod :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument paymentMethod */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createPaymentMethod', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createPaymentMethod', payload))
    .then(payload => {
      ...
      payload.id = uuid.v4()
      payload.href = "http://localhost:8670/tmf-api/paymentMethod/v4/paymentMethod/" + payload.id
    ...
    .catch( error => {
      console.log("createPaymentMethod: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deletePaymentMethod = function(req, res, next) {
  /**
  ...
  console.log('deletePaymentMethod :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listPaymentMethod = function(req, res, next) {
  /**
  ...
  console.log('listPaymentMethod :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  if (query.criteria.statusDate) {
    query.criteria.statusDate = query.criteria.statusDate.toJSON()
  }
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listPaymentMethod: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchPaymentMethod = function(req, res, next) {
  /**
   * Updates partially a PaymentMethod
   * This operation updates partially a PaymentMethod entity.
   *
   * id String Identifier of the PaymentMethod
   * paymentMethod PaymentMethod_Update The PaymentMethod to be updated
   * returns PaymentMethod
   **/

  console.log('patchPaymentMethod :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchPaymentMethod',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchPaymentMethod error=" + error);
...
exports.retrievePaymentMethod = function(req, res, next) {
  /**
  ...
  console.log('retrievePaymentMethod :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrievePaymentMethod: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
