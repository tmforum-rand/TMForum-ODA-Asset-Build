...
exports.listBillCycleSpecification = function(req, res, next) {
  /**
   * List or find BillCycleSpecification objects
   * This operation list or find BillCycleSpecification entities
   *
  ...
  console.log('listBillCycleSpecification :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listBillCycleSpecification: error=" + error);
...
exports.retrieveBillCycleSpecification = function(req, res, next) {
  /**
   * Retrieves a BillCycleSpecification by ID
   * This operation retrieves a BillCycleSpecification entity. Attribute selection is enabled for all first level attributes.
   *
   * id String Identifier of the BillCycleSpecification
   * fields String Comma-separated properties to provide in response (optional)
   * returns BillCycleSpecification
   **/

  console.log('retrieveBillCycleSpecification :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveBillCycleSpecification: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
