...
exports.createSalesLead = function(req, res, next) {
  /**
   * Creates a SalesLead
   * This operation creates a SalesLead entity.
   *
   * salesLead SalesLead_Create The SalesLead to be created
   * returns SalesLead
   **/

  console.log('createSalesLead :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument salesLead */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createSalesLead', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createSalesLead', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createSalesLead: error=" + error);
    ...
    .catch( error => {
      console.log("createSalesLead: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteSalesLead = function(req, res, next) {
  /**
   * Deletes a SalesLead
  ...
  console.log('deleteSalesLead :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listSalesLead = function(req, res, next) {
  /**
  ...
  console.log('listSalesLead :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listSalesLead: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchSalesLead = function(req, res, next) {
  /**
   * Updates partially a SalesLead
   * This operation updates partially a SalesLead entity.
   *
   * id String Identifier of the SalesLead
   * salesLead SalesLead_Update The SalesLead to be updated
   * returns SalesLead
   **/

  console.log('patchSalesLead :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchSalesLead',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchSalesLead error=" + error);
...
exports.retrieveSalesLead = function(req, res, next) {
  /**
  ...
  console.log('retrieveSalesLead :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveSalesLead: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
