...
exports.createTestCase = function(req, res, next) {
  /**
   * Creates a TestCase
   * This operation creates a TestCase entity.
   *
   * testCase TestCase_Create The TestCase to be created
   * returns TestCase
   **/

  console.log('createTestCase :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument testCase */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createTestCase', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createTestCase', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createTestCase: error=" + error);
    ...
    .catch( error => {
      console.log("createTestCase: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteTestCase = function(req, res, next) {
  /**
   * Deletes a TestCase
  ...
  console.log('deleteTestCase :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listTestCase = function(req, res, next) {
  /**
  ...
  console.log('listTestCase :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listTestCase: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchTestCase = function(req, res, next) {
  /**
   * Updates partially a TestCase
   * This operation updates partially a TestCase entity.
   *
   * id String Identifier of the TestCase
   * testCase TestCase_Update The TestCase to be updated
   * returns TestCase
   **/

  console.log('patchTestCase :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchTestCase',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchTestCase error=" + error);
...
exports.retrieveTestCase = function(req, res, next) {
  /**
  ...
  console.log('retrieveTestCase :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveTestCase: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
