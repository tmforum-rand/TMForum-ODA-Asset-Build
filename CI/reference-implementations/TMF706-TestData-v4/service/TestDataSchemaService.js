...
exports.createTestDataSchema = function(req, res, next) {
  /**
   * Creates a TestDataSchema
   * This operation creates a TestDataSchema entity.
   *
   * testDataSchema TestDataSchema_Create The TestDataSchema to be created
   * returns TestDataSchema
   **/

  console.log('createTestDataSchema :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument testDataSchema */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createTestDataSchema', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createTestDataSchema', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createTestDataSchema: error=" + error);
    ...
    .catch( error => {
      console.log("createTestDataSchema: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteTestDataSchema = function(req, res, next) {
  /**
  ...
  console.log('deleteTestDataSchema :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listTestDataSchema = function(req, res, next) {
  /**
  ...
  console.log('listTestDataSchema :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listTestDataSchema: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchTestDataSchema = function(req, res, next) {
  /**
   * Updates partially a TestDataSchema
   * This operation updates partially a TestDataSchema entity.
   *
   * id String Identifier of the TestDataSchema
   * testDataSchema TestDataSchema_Update The TestDataSchema to be updated
   * returns TestDataSchema
   **/

  console.log('patchTestDataSchema :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchTestDataSchema',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchTestDataSchema error=" + error);
...
exports.retrieveTestDataSchema = function(req, res, next) {
  /**
  ...
  console.log('retrieveTestDataSchema :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveTestDataSchema: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
