...
exports.createChangeRequest = function(req, res, next) {
  /**
   * Creates a ChangeRequest
   * This operation creates a ChangeRequest entity.
   *
   * changeRequest ChangeRequest_Create The ChangeRequest to be created
   * returns ChangeRequest
   **/

  console.log('createChangeRequest :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument changeRequest */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createChangeRequest', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createChangeRequest', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createChangeRequest: error=" + error);
    ...
    .catch( error => {
      console.log("createChangeRequest: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteChangeRequest = function(req, res, next) {
  /**
  ...
  console.log('deleteChangeRequest :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listChangeRequest = function(req, res, next) {
  /**
  ...
  console.log('listChangeRequest :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  if (query.criteria.plannedEndTime) {
    query.criteria.plannedEndTime = query.criteria.plannedEndTime.toJSON()
  }
  if (query.criteria.plannedStartTime) {
    query.criteria.plannedStartTime = query.criteria.plannedStartTime.toJSON()
  }
  mongoUtils.connect()
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listChangeRequest: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchChangeRequest = function(req, res, next) {
  /**
   * Updates partially a ChangeRequest
   * This operation updates partially a ChangeRequest entity.
   *
   * id String Identifier of the ChangeRequest
   * changeRequest ChangeRequest_Update The ChangeRequest to be updated
   * returns ChangeRequest
   **/

  console.log('patchChangeRequest :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchChangeRequest',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchChangeRequest error=" + error);
...
exports.retrieveChangeRequest = function(req, res, next) {
  /**
  ...
  console.log('retrieveChangeRequest :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveChangeRequest: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
