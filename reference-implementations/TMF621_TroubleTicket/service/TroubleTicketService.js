...
exports.createTroubleTicket = function(req, res, next) {
  /**
   * Creates a TroubleTicket
   * This operation creates a TroubleTicket entity.
   *
   * troubleTicket TroubleTicket_Create The TroubleTicket to be created
   * returns TroubleTicket
   **/

  console.log('createTroubleTicket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument troubleTicket */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createTroubleTicket', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createTroubleTicket', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createTroubleTicket: error=" + error);
    ...
    .catch( error => {
      console.log("createTroubleTicket: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteTroubleTicket = function(req, res, next) {
  /**
  ...
  console.log('deleteTroubleTicket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listTroubleTicket = function(req, res, next) {
  /**
  ...
  console.log('listTroubleTicket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listTroubleTicket: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchTroubleTicket = function(req, res, next) {
  /**
   * Updates partially a TroubleTicket
   * This operation updates partially a TroubleTicket entity.
   *
   * id String Identifier of the TroubleTicket
   * troubleTicket TroubleTicket_Update The TroubleTicket to be updated
   * returns TroubleTicket
   **/

  console.log('patchTroubleTicket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchTroubleTicket',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchTroubleTicket error=" + error);
...
exports.retrieveTroubleTicket = function(req, res, next) {
  /**
  ...
  console.log('retrieveTroubleTicket :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveTroubleTicket: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
