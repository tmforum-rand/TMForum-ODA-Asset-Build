...
exports.createTestCaseResult = function(req, res, next) {
  /**
   * Creates a TestCaseResult
   * This operation creates a TestCaseResult entity.
   *
   * testCaseResult TestCaseResult_Create The TestCaseResult to be created
   * returns TestCaseResult
   **/

  console.log('createTestCaseResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument testCaseResult */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createTestCaseResult', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createTestCaseResult', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createTestCaseResult: error=" + error);
    ...
    .catch( error => {
      console.log("createTestCaseResult: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteTestCaseResult = function(req, res, next) {
  /**
  ...
  console.log('deleteTestCaseResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listTestCaseResult = function(req, res, next) {
  /**
  ...
  console.log('listTestCaseResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listTestCaseResult: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchTestCaseResult = function(req, res, next) {
  /**
   * Updates partially a TestCaseResult
   * This operation updates partially a TestCaseResult entity.
   *
   * id String Identifier of the TestCaseResult
   * testCaseResult TestCaseResult_Update The TestCaseResult to be updated
   * returns TestCaseResult
   **/

  console.log('patchTestCaseResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchTestCaseResult',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchTestCaseResult error=" + error);
...
exports.retrieveTestCaseResult = function(req, res, next) {
  /**
  ...
  console.log('retrieveTestCaseResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveTestCaseResult: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
