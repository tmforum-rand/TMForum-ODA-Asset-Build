...
exports.createTestSuiteResult = function(req, res, next) {
  /**
   * Creates a TestSuiteResult
   * This operation creates a TestSuiteResult entity.
   *
   * testSuiteResult TestSuiteResult_Create The TestSuiteResult to be created
   * returns TestSuiteResult
   **/

  console.log('createTestSuiteResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument testSuiteResult */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createTestSuiteResult', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createTestSuiteResult', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createTestSuiteResult: error=" + error);
    ...
    .catch( error => {
      console.log("createTestSuiteResult: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteTestSuiteResult = function(req, res, next) {
  /**
  ...
  console.log('deleteTestSuiteResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listTestSuiteResult = function(req, res, next) {
  /**
  ...
  console.log('listTestSuiteResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listTestSuiteResult: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchTestSuiteResult = function(req, res, next) {
  /**
   * Updates partially a TestSuiteResult
   * This operation updates partially a TestSuiteResult entity.
   *
   * id String Identifier of the TestSuiteResult
   * testSuiteResult TestSuiteResult_Update The TestSuiteResult to be updated
   * returns TestSuiteResult
   **/

  console.log('patchTestSuiteResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchTestSuiteResult',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchTestSuiteResult error=" + error);
...
exports.retrieveTestSuiteResult = function(req, res, next) {
  /**
  ...
  console.log('retrieveTestSuiteResult :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveTestSuiteResult: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
