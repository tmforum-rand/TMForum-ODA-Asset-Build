...
exports.createGeographicSite = function(req, res, next) {
  /**
   * Creates a GeographicSite
   * This operation creates a GeographicSite entity.
   *
   * geographicSite GeographicSite_Create The GeographicSite to be created
   * returns GeographicSite
   **/

  console.log('createGeographicSite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument geographicSite */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createGeographicSite', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createGeographicSite', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createGeographicSite: error=" + error);
    ...
    .catch( error => {
      console.log("createGeographicSite: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteGeographicSite = function(req, res, next) {
  /**
  ...
  console.log('deleteGeographicSite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listGeographicSite = function(req, res, next) {
  /**
  ...
  console.log('listGeographicSite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listGeographicSite: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchGeographicSite = function(req, res, next) {
  /**
   * Updates partially a GeographicSite
   * This operation updates partially a GeographicSite entity.
   *
   * id String Identifier of the GeographicSite
   * geographicSite GeographicSite_Update The GeographicSite to be updated
   * returns GeographicSite
   **/

  console.log('patchGeographicSite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchGeographicSite',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchGeographicSite error=" + error);
...
exports.retrieveGeographicSite = function(req, res, next) {
  /**
  ...
  console.log('retrieveGeographicSite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveGeographicSite: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
