...
exports.createAlarm = function(req, res, next) {
   ...
   *
   * alarm Alarm_Create The Alarm to be created
   * returns Alarm
   **/

  console.log('createAlarm :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument alarm */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createAlarm', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createAlarm', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createAlarm: error=" + error);
    ...
    .catch( error => {
      console.log("createAlarm: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteAlarm = function(req, res, next) {
  ...
  console.log('deleteAlarm :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listAlarm = function(req, res, next) {
  /**
  ...
  console.log('listAlarm :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listAlarm: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchAlarm = function(req, res, next) {
  /**
   * Updates partially a Alarm
   * This operation updates partially a Alarm entity.
   *
   * id String Identifier of the Alarm
   * alarm Alarm_Update The Alarm to be updated
   * returns Alarm
   **/

  console.log('patchAlarm :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchAlarm',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchAlarm error=" + error);
...
exports.retrieveAlarm = function(req, res, next) {
  /**
  ...
  console.log('retrieveAlarm :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveAlarm: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
