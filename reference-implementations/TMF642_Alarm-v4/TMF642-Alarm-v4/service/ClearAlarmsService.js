...
exports.createClearAlarms = function(req, res, next) {
  /**
   * Creates a ClearAlarms
   * This operation creates a ClearAlarms entity.
   *
   * clearAlarms ClearAlarms_Create The ClearAlarms to be created
   * returns ClearAlarms
   **/

  console.log('createClearAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument clearAlarms */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createClearAlarms', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createClearAlarms', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createClearAlarms: error=" + error);
    ...
    .catch( error => {
      console.log("createClearAlarms: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listClearAlarms = function(req, res, next) {
  /**
  ...
  console.log('listClearAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listClearAlarms: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveClearAlarms = function(req, res, next) {
  /**
  ...
  console.log('retrieveClearAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveClearAlarms: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
