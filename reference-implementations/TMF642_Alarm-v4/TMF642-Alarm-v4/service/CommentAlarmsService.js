...
exports.createCommentAlarms = function(req, res, next) {
  /**
   * Creates a CommentAlarms
   * This operation creates a CommentAlarms entity.
   *
   * commentAlarms CommentAlarms_Create The CommentAlarms to be created
   * returns CommentAlarms
   **/

  console.log('createCommentAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument commentAlarms */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createCommentAlarms', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createCommentAlarms', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createCommentAlarms: error=" + error);
    ...
    .catch( error => {
      console.log("createCommentAlarms: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listCommentAlarms = function(req, res, next) {
  /**
  ...
  console.log('listCommentAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listCommentAlarms: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveCommentAlarms = function(req, res, next) {
  /**
  ...
  console.log('retrieveCommentAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveCommentAlarms: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
