...
exports.createGroupAlarms = function(req, res, next) {
  /**
   * Creates a GroupAlarms
   * This operation creates a GroupAlarms entity.
   *
   * groupAlarms GroupAlarms_Create The GroupAlarms to be created
   * returns GroupAlarms
   **/

  console.log('createGroupAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument groupAlarms */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createGroupAlarms', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createGroupAlarms', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createGroupAlarms: error=" + error);
    ...
    .catch( error => {
      console.log("createGroupAlarms: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listGroupAlarms = function(req, res, next) {
  /**
  ...
  console.log('listGroupAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listGroupAlarms: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveGroupAlarms = function(req, res, next) {
  /**
  ...
  console.log('retrieveGroupAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveGroupAlarms: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
