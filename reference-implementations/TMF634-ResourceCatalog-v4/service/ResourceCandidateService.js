...
exports.createResourceCandidate = function(req, res, next) {
  /**
   * Creates a ResourceCandidate
   * This operation creates a ResourceCandidate entity.
   *
   * resourceCandidate ResourceCandidate_Create The ResourceCandidate to be created
   * returns ResourceCandidate
   **/

  console.log('createResourceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument resourceCandidate */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createResourceCandidate', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createResourceCandidate', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createResourceCandidate: error=" + error);
    ...
    .catch( error => {
      console.log("createResourceCandidate: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteResourceCandidate = function(req, res, next) {
  /**
  ...
  console.log('deleteResourceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listResourceCandidate = function(req, res, next) {
  /**
  ...
  console.log('listResourceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listResourceCandidate: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchResourceCandidate = function(req, res, next) {
  /**
   * Updates partially a ResourceCandidate
   * This operation updates partially a ResourceCandidate entity.
   *
   * id String Identifier of the ResourceCandidate
   * resourceCandidate ResourceCandidate_Update The ResourceCandidate to be updated
   * returns ResourceCandidate
   **/

  console.log('patchResourceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchResourceCandidate',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchResourceCandidate error=" + error);
...
exports.retrieveResourceCandidate = function(req, res, next) {
  /**
  ...
  console.log('retrieveResourceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveResourceCandidate: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
