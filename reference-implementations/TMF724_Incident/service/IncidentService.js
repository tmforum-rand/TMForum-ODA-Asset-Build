...
exports.createIncident = function(req, res, next) {
   ...
   *
   * incident Incident_Create The Incident to be created
   * returns Incident
   **/
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createIncident', payload))
    ...
    .catch( error => {
      console.log("createIncident: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listIncident = function(req, res, next) {
  /**
  ...
  console.log('listIncident :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listIncident: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveIncident = function(req, res, next) {
  /**
   ...
   *
   * id String Identifier of the Incident
   * fields String Comma-separated properties to provide in response (optional)
   * returns Incident
   **/

  console.log('retrieveIncident :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveIncident: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
