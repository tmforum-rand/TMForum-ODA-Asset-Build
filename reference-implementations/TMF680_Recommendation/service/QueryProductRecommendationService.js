...
exports.createQueryProductRecommendation = function(req, res, next) {
   ...
   *
   * queryProductRecommendation QueryProductRecommendation_Create The QueryProductRecommendation to be created
   * returns QueryProductRecommendation
   **/

  console.log('createQueryProductRecommendation :: ' + req.method + ' ' + req.url + ' ' + req.headers.host + " mandatoryget " + req.headers.mandatoryget);

  /* matching isRestfulCreate - argument queryProductRecommendation */
  ...
  swaggerUtils.getPayload(req)
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .then(() => {
            ...
            console.log("mandatory get", req.headers.mandatoryget)
            let statusCode = 201
            if (req.headers.mandatoryget === "true") {
              statusCode = 200
            }
          ...
          .catch((error) => {
            console.log("createQueryProductRecommendation: error=" + error);
    ...
    .catch( error => {
      console.log("createQueryProductRecommendation: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listQueryProductRecommendation = function(req, res, next) {
  ...
  console.log('listQueryProductRecommendation :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listQueryProductRecommendation: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveQueryProductRecommendation = function(req, res, next) {
  ...
  console.log('retrieveQueryProductRecommendation :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveQueryProductRecommendation: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
