...
exports.createReserveBalance = function(req, res, next) {
  /**
   * Creates a ReserveBalance
   * This operation creates a ReserveBalance entity.
   *
   * reserveBalance ReserveBalance_Create The ReserveBalance to be created
   * returns ReserveBalance
   **/

  console.log('createReserveBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument reserveBalance */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createReserveBalance', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createReserveBalance', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createReserveBalance: error=" + error);
    ...
    .catch( error => {
      console.log("createReserveBalance: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteReserveBalance = function(req, res, next) {
  /**
  ...
  console.log('deleteReserveBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listReserveBalance = function(req, res, next) {
  /**
  ...
  console.log('listReserveBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listReserveBalance: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchReserveBalance = function(req, res, next) {
  /**
   * Updates partially a ReserveBalance
   * This operation updates partially a ReserveBalance entity.
   *
   * id String Identifier of the ReserveBalance
   * reserveBalance ReserveBalance_Update The ReserveBalance to be updated
   * returns ReserveBalance
   **/

  console.log('patchReserveBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchReserveBalance',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchReserveBalance error=" + error);
...
exports.retrieveReserveBalance = function(req, res, next) {
  /**
  ...
  console.log('retrieveReserveBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveReserveBalance: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
