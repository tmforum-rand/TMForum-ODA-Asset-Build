...
exports.createDiagnoseIncident = function(req, res, next) {
  /**
   * Creates a DiagnoseIncident
   * This operation creates a DiagnoseIncident entity.
   *
   * diagnoseIncident DiagnoseIncident_Create The DiagnoseIncident to be created
   * returns DiagnoseIncident
   **/

  console.log('createDiagnoseIncident :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument diagnoseIncident */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createDiagnoseIncident', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createDiagnoseIncident', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createDiagnoseIncident: error=" + error);
    ...
    .catch( error => {
      console.log("createDiagnoseIncident: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listDiagnoseIncident = function(req, res, next) {
  /**
  ...
  console.log('listDiagnoseIncident :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listDiagnoseIncident: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveDiagnoseIncident = function(req, res, next) {
  /**
   ...
   *
   * id String Identifier of the DiagnoseIncident
   * fields String Comma-separated properties to provide in response (optional)
   * returns DiagnoseIncident
   **/

  console.log('retrieveDiagnoseIncident :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveDiagnoseIncident: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
