...
exports.createPayment = function(req, res, next) {
   ...
   *
   * payment Payment_Create The Payment to be created
   * returns Payment
   **/

  console.log('createPayment :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument payment */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createPayment', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createPayment', payload))
    .then(payload => {
    ...
    .catch( error => {
      console.log("createPayment: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listPayment = function(req, res, next) {
  ...
  console.log('listPayment :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listPayment: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrievePayment = function(req, res, next) {
  /**
  ...
  console.log('retrievePayment :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
