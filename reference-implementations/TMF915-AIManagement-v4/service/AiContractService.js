...
exports.createAiContract = function(req, res, next) {
  /**
   * Creates a AiContract
   * This operation creates a AiContract entity.
   *
   * aiContract AiContract_Create The AiContract to be created
   * returns AiContract
   **/

  console.log('createAiContract :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument aiContract */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createAiContract', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createAiContract', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createAiContract: error=" + error);
    ...
    .catch( error => {
      console.log("createAiContract: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteAiContract = function(req, res, next) {
  /**
  ...
  console.log('deleteAiContract :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listAiContract = function(req, res, next) {
  /**
  ...
  console.log('listAiContract :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listAiContract: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchAiContract = function(req, res, next) {
  /**
   * Updates partially a AiContract
   * This operation updates partially a AiContract entity.
   *
   * id String Identifier of the AiContract
   * aiContract AiContract_Update The AiContract to be updated
   * returns AiContract
   **/

  console.log('patchAiContract :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchAiContract',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchAiContract error=" + error);
...
exports.retrieveAiContract = function(req, res, next) {
  /**
  ...
  console.log('retrieveAiContract :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveAiContract: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
