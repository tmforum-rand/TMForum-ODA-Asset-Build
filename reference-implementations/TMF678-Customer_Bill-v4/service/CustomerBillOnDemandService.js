...
exports.createCustomerBillOnDemand = function(req, res, next) {
  /**
   * Creates a CustomerBillOnDemand
   * This operation creates a CustomerBillOnDemand entity.
   *
   * customerBillOnDemand CustomerBillOnDemand_Create The CustomerBillOnDemand to be created
   * returns CustomerBillOnDemand
   **/

  console.log('createCustomerBillOnDemand :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument customerBillOnDemand */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createCustomerBillOnDemand', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createCustomerBillOnDemand', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createCustomerBillOnDemand: error=" + error);
    ...
    .catch( error => {
      console.log("createCustomerBillOnDemand: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listCustomerBillOnDemand = function(req, res, next) {
  /**
  ...
  console.log('listCustomerBillOnDemand :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listCustomerBillOnDemand: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveCustomerBillOnDemand = function(req, res, next) {
  /**
  ...
  console.log('retrieveCustomerBillOnDemand :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveCustomerBillOnDemand: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
