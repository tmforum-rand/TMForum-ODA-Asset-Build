...
exports.createAdjustProductStock = function(req, res, next) {
  /**
   * Creates a AdjustProductStock
   * This operation creates a AdjustProductStock entity.
   *
   * adjustProductStock AdjustProductStock_Create The AdjustProductStock to be created
   * returns AdjustProductStock
   **/

  console.log('createAdjustProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument adjustProductStock */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createAdjustProductStock', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createAdjustProductStock', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createAdjustProductStock: error=" + error);
    ...
    .catch( error => {
      console.log("createAdjustProductStock: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listAdjustProductStock = function(req, res, next) {
  /**
  ...
  console.log('listAdjustProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listAdjustProductStock: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveAdjustProductStock = function(req, res, next) {
  /**
  ...
  console.log('retrieveAdjustProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveAdjustProductStock: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
