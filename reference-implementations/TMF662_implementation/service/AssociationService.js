...
exports.createAssociation = function(req, res, next) {
  /**
   * Creates a Association
   * This operation creates a Association entity.
   *
   * association Association_Create The Association to be created
   * returns Association
   **/

  console.log('createAssociation :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument association */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createAssociation', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createAssociation', payload))
    .then(payload => {
    ...
    .catch( error => {
      console.log("createAssociation: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteAssociation = function(req, res, next) {
  /**
  ...
  console.log('deleteAssociation :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listAssociation = function(req, res, next) {
  /**
  ...
  console.log('listAssociation :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listAssociation: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchAssociation = function(req, res, next) {
  /**
   * Updates partially a Association
   * This operation updates partially a Association entity.
   *
   * id String Identifier of the Association
   * association Association_Update The Association to be updated
   * returns Association
   **/

  console.log('patchAssociation :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchAssociation',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchAssociation error=" + error);
...
exports.retrieveAssociation = function(req, res, next) {
  /**
  ...
  console.log('retrieveAssociation :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveAssociation: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
