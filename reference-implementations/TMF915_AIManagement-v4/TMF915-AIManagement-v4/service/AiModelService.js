...
exports.createAiModel = function(req, res, next) {
  /**
   * Creates a AiModel
   * This operation creates a AiModel entity.
   *
   * aiModel AiModel_Create The AiModel to be created
   * returns AiModel
   **/

  console.log('createAiModel :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument aiModel */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createAiModel', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createAiModel', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createAiModel: error=" + error);
    ...
    .catch( error => {
      console.log("createAiModel: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteAiModel = function(req, res, next) {
  /**
   * Deletes a AiModel
  ...
  console.log('deleteAiModel :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listAiModel = function(req, res, next) {
  /**
  ...
  console.log('listAiModel :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listAiModel: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchAiModel = function(req, res, next) {
  /**
   * Updates partially a AiModel
   * This operation updates partially a AiModel entity.
   *
   * id String Identifier of the AiModel
   * aiModel AiModel_Update The AiModel to be updated
   * returns AiModel
   **/

  console.log('patchAiModel :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchAiModel',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchAiModel error=" + error);
...
exports.retrieveAiModel = function(req, res, next) {
  /**
  ...
  console.log('retrieveAiModel :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveAiModel: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
