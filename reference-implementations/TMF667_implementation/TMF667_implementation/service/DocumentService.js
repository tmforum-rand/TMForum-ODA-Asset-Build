...
exports.createDocument = function(req, res, next) {
   ...
   *
   * document Document_Create The Document to be created
   * returns Document
   **/

  console.log('createDocument :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument document */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createDocument', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createDocument', payload))
    .then(payload => {
    ...
    .catch( error => {
      console.log("createDocument: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteDocument = function(req, res, next) {
  ...
  console.log('deleteDocument :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listDocument = function(req, res, next) {
   ...
   * This operation list or find Document entities
   *
  ...
  console.log('listDocument :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listDocument: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchDocument = function(req, res, next) {
  /**
   * Updates partially a Document
   * This operation updates partially a Document entity.
   *
   * id String Identifier of the Document
   * document Document_Update The Document to be updated
   * returns Document
   **/

  console.log('patchDocument :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchDocument',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchDocument error=" + error);
...
exports.retrieveDocument = function(req, res, next) {
  /**
   * Retrieves a Document by ID
   * This operation retrieves a Document entity. Attribute selection is enabled for all first level attributes.
   *
   * id String Identifier of the Document
  ...
  console.log('retrieveDocument :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveDocument: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
