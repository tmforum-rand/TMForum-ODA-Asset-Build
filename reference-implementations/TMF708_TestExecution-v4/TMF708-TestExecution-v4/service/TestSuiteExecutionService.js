...
exports.createTestSuiteExecution = function(req, res, next) {
  /**
   * Creates a TestSuiteExecution
   * This operation creates a TestSuiteExecution entity.
   *
   * testSuiteExecution TestSuiteExecution_Create The TestSuiteExecution to be created
   * returns TestSuiteExecution
   **/

  console.log('createTestSuiteExecution :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument testSuiteExecution */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createTestSuiteExecution', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createTestSuiteExecution', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createTestSuiteExecution: error=" + error);
    ...
    .catch( error => {
      console.log("createTestSuiteExecution: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteTestSuiteExecution = function(req, res, next) {
  /**
  ...
  console.log('deleteTestSuiteExecution :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listTestSuiteExecution = function(req, res, next) {
  /**
  ...
  console.log('listTestSuiteExecution :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listTestSuiteExecution: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveTestSuiteExecution = function(req, res, next) {
  /**
  ...
  console.log('retrieveTestSuiteExecution :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveTestSuiteExecution: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
