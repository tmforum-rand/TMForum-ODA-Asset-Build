...
exports.createScale = function(req, res, next) {
  /**
   * Creates a Scale
   * This operation creates a Scale entity.
   *
   * scale Scale_Create The Scale to be created
   * returns Scale
   **/

  console.log('createScale :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument scale */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createScale', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createScale', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createScale: error=" + error);
    ...
    .catch( error => {
      console.log("createScale: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listScale = function(req, res, next) {
  /**
  ...
  console.log('listScale :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listScale: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveScale = function(req, res, next) {
  /**
  ...
  console.log('retrieveScale :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveScale: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
