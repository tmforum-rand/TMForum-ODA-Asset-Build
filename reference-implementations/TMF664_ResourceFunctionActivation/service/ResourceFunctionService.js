...
exports.createResourceFunction = function(req, res, next) {
  /**
   * Creates a ResourceFunction
   * This operation creates a ResourceFunction entity.
   *
   * resourceFunction ResourceFunction_Create The ResourceFunction to be created
   * returns ResourceFunction
   **/

  console.log('createResourceFunction :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument resourceFunction */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createResourceFunction', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createResourceFunction', payload))
    .then(payload => {
      // Check if ri should return async monitor
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .then(() => {
            ...
            console.log("RUN_ASYNC", req.headers["run_async"])
            console.log(req.headers)
            if (req.headers["run_async"] === "true"){
              let monitorPayload = {
                href: "/tmf-api/resourceFunctionActivation/v4/Monitor/123123",
                id: "123123",
                state: "InProgress",
                sourceHref: payload.href
              }
                db.collection("Monitor")
                  .insertOne(monitorPayload)
                  .then(() => {
                    sendDoc(res, 202, monitorPayload)
                  })
            }
            else{
          ...
          .catch((error) => {
            console.log("createResourceFunction: error=" + error);
    ...
    .catch( error => {
      console.log("createResourceFunction: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteResourceFunction = function(req, res, next) {
  /**
  ...
  console.log('deleteResourceFunction :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listResourceFunction = function(req, res, next) {
  /**
  ...
  console.log('listResourceFunction :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listResourceFunction: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchResourceFunction = function(req, res, next) {
  /**
   * Updates partially a ResourceFunction
   * This operation updates partially a ResourceFunction entity.
   *
   * id String Identifier of the ResourceFunction
   * resourceFunction ResourceFunction_Update The ResourceFunction to be updated
   * returns ResourceFunction
   **/

  console.log('patchResourceFunction :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchResourceFunction',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchResourceFunction error=" + error);
...
exports.retrieveResourceFunction = function(req, res, next) {
  /**
  ...
  console.log('retrieveResourceFunction :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveResourceFunction: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
