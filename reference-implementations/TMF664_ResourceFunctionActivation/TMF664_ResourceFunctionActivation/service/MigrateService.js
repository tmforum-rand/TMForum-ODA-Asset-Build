...
exports.createMigrate = function(req, res, next) {
  /**
   * Creates a Migrate
   * This operation creates a Migrate entity.
   *
   * migrate Migrate_Create The Migrate to be created
   * returns Migrate
   **/

  console.log('createMigrate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument migrate */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createMigrate', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createMigrate', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createMigrate: error=" + error);
    ...
    .catch( error => {
      console.log("createMigrate: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listMigrate = function(req, res, next) {
  /**
  ...
  console.log('listMigrate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listMigrate: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveMigrate = function(req, res, next) {
  /**
  ...
  console.log('retrieveMigrate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveMigrate: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
