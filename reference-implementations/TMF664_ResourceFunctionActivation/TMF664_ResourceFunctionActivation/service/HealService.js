...
exports.createHeal = function(req, res, next) {
  /**
   * Creates a Heal
   * This operation creates a Heal entity.
   *
   * heal Heal_Create The Heal to be created
   * returns Heal
   **/

  console.log('createHeal :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument heal */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createHeal', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createHeal', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createHeal: error=" + error);
    ...
    .catch( error => {
      console.log("createHeal: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listHeal = function(req, res, next) {
  /**
  ...
  console.log('listHeal :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listHeal: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveHeal = function(req, res, next) {
  /**
  ...
  console.log('retrieveHeal :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveHeal: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
