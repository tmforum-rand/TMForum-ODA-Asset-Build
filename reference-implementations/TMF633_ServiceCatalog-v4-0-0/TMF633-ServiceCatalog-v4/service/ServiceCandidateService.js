...
exports.createServiceCandidate = function(req, res, next) {
   ...
   *
   * serviceCandidate ServiceCandidate_Create The ServiceCandidate to be created
   * returns ServiceCandidate
   **/

  console.log('createServiceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument serviceCandidate */
  ...
  swaggerUtils.getPayload(req)
    ...
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createServiceCandidate: error=" + error);
    ...
    .catch( error => {
      console.log("createServiceCandidate: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteServiceCandidate = function(req, res, next) {
  ...
  console.log('deleteServiceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listServiceCandidate = function(req, res, next) {
  ...
  console.log('listServiceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listServiceCandidate: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchServiceCandidate = function(req, res, next) {
   ...
   *
   * id String Identifier of the ServiceCandidate
   * serviceCandidate ServiceCandidate_Update The ServiceCandidate to be updated
   * returns ServiceCandidate
   **/

  console.log('patchServiceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchServiceCandidate error=" + error);
...
exports.retrieveServiceCandidate = function(req, res, next) {
  ...
  console.log('retrieveServiceCandidate :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveServiceCandidate: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
