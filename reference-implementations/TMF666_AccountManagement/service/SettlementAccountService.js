...
exports.createSettlementAccount = function(req, res, next) {
  /**
   * Creates a SettlementAccount
   * This operation creates a SettlementAccount entity.
   *
   * settlementAccount SettlementAccount_Create The SettlementAccount to be created
   * returns SettlementAccount
   **/

  console.log('createSettlementAccount :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument settlementAccount */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createSettlementAccount', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createSettlementAccount', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createSettlementAccount: error=" + error);
    ...
    .catch( error => {
      console.log("createSettlementAccount: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteSettlementAccount = function(req, res, next) {
  /**
  ...
  console.log('deleteSettlementAccount :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listSettlementAccount = function(req, res, next) {
  /**
  ...
  console.log('listSettlementAccount :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listSettlementAccount: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchSettlementAccount = function(req, res, next) {
  /**
   * Updates partially a SettlementAccount
   * This operation updates partially a SettlementAccount entity.
   *
   * id String Identifier of the SettlementAccount
   * settlementAccount SettlementAccount_Update The SettlementAccount to be updated
   * returns SettlementAccount
   **/

  console.log('patchSettlementAccount :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchSettlementAccount',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchSettlementAccount error=" + error);
...
exports.retrieveSettlementAccount = function(req, res, next) {
  /**
  ...
  console.log('retrieveSettlementAccount :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveSettlementAccount: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
