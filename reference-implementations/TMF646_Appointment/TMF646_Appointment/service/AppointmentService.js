...
exports.createAppointment = function(req, res, next) {
   ...
   *
   * appointment Appointment_Create The Appointment to be created
   * returns Appointment
   **/

  console.log('createAppointment :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument appointment */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createAppointment', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createAppointment', payload))
    .then(payload => {
    ...
    .catch( error => {
      console.log("createAppointment: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteAppointment = function(req, res, next) {
  ...
  console.log('deleteAppointment :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listAppointment = function(req, res, next) {
  ...
  console.log('listAppointment :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.patchAppointment = function(req, res, next) {
  /**
   * Updates partially a Appointment
   * This operation updates partially a Appointment entity.
   *
   * id String Identifier of the Appointment
   * appointment Appointment_Update The Appointment to be updated
   * returns Appointment
   **/

  console.log('patchAppointment :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchAppointment',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchAppointment error=" + error);
...
exports.retrieveAppointment = function(req, res, next) {
  /**
  ...
  console.log('retrieveAppointment :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveAppointment: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
