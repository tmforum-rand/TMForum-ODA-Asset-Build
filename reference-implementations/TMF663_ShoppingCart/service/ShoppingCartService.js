...
exports.createShoppingCart = function(req, res, next) {
  /**
   * Creates a ShoppingCart
   * This operation creates a ShoppingCart entity.
   *
   * shoppingCart ShoppingCart_Create The ShoppingCart to be created
   * returns ShoppingCart
   **/

  console.log('createShoppingCart :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument shoppingCart */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createShoppingCart', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createShoppingCart', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createShoppingCart: error=" + error);
    ...
    .catch( error => {
      console.log("createShoppingCart: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteShoppingCart = function(req, res, next) {
  /**
  ...
  console.log('deleteShoppingCart :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listShoppingCart = function(req, res, next) {
  /**
  ...
  console.log('listShoppingCart :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.patchShoppingCart = function(req, res, next) {
  /**
   * Updates partially a ShoppingCart
   * This operation updates partially a ShoppingCart entity.
   *
   * id String Identifier of the ShoppingCart
   * shoppingCart ShoppingCart_Update The ShoppingCart to be updated
   * returns ShoppingCart
   **/

  console.log('patchShoppingCart :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchShoppingCart',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchShoppingCart error=" + error);
...
exports.retrieveShoppingCart = function(req, res, next) {
  /**
  ...
  console.log('retrieveShoppingCart :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveShoppingCart: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
