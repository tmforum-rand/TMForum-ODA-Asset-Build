...
exports.createEntityCatalog = function(req, res, next) {
  /**
   * Creates a EntityCatalog
   * This operation creates a EntityCatalog entity.
   *
   * entityCatalog EntityCatalog_Create The EntityCatalog to be created
   * returns EntityCatalog
   **/

  console.log('createEntityCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument entityCatalog */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createEntityCatalog', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createEntityCatalog', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createEntityCatalog: error=" + error);
    ...
    .catch( error => {
      console.log("createEntityCatalog: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteEntityCatalog = function(req, res, next) {
  /**
  ...
  console.log('deleteEntityCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listEntityCatalog = function(req, res, next) {
  /**
  ...
  console.log('listEntityCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listEntityCatalog: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchEntityCatalog = function(req, res, next) {
  /**
   * Updates partially a EntityCatalog
   * This operation updates partially a EntityCatalog entity.
   *
   * id String Identifier of the EntityCatalog
   * entityCatalog EntityCatalog_Update The EntityCatalog to be updated
   * returns EntityCatalog
   **/

  console.log('patchEntityCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchEntityCatalog',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchEntityCatalog error=" + error);
...
exports.retrieveEntityCatalog = function(req, res, next) {
  /**
  ...
  console.log('retrieveEntityCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveEntityCatalog: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
