...
exports.createTopupBalance = function(req, res, next) {
  /**
   * Creates a TopupBalance
   * This operation creates a TopupBalance entity.
   *
   * topupBalance TopupBalance_Create The TopupBalance to be created
   * returns TopupBalance
   **/

  console.log('createTopupBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument topupBalance */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createTopupBalance', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createTopupBalance', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createTopupBalance: error=" + error);
    ...
    .catch( error => {
      console.log("createTopupBalance: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteTopupBalance = function(req, res, next) {
  /**
  ...
  console.log('deleteTopupBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listTopupBalance = function(req, res, next) {
  /**
  ...
  console.log('listTopupBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listTopupBalance: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchTopupBalance = function(req, res, next) {
  /**
   * Updates partially a TopupBalance
   * This operation updates partially a TopupBalance entity.
   *
   * id String Identifier of the TopupBalance
   * topupBalance TopupBalance_Update The TopupBalance to be updated
   * returns TopupBalance
   **/

  console.log('patchTopupBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchTopupBalance',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchTopupBalance error=" + error);
...
exports.retrieveTopupBalance = function(req, res, next) {
  /**
  ...
  console.log('retrieveTopupBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveTopupBalance: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
