...
exports.createAdjustBalance = function(req, res, next) {
  /**
   * Creates a AdjustBalance
   * This operation creates a AdjustBalance entity.
   *
   * adjustBalance AdjustBalance_Create The AdjustBalance to be created
   * returns AdjustBalance
   **/

  console.log('createAdjustBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument adjustBalance */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createAdjustBalance', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createAdjustBalance', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createAdjustBalance: error=" + error);
    ...
    .catch( error => {
      console.log("createAdjustBalance: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteAdjustBalance = function(req, res, next) {
  /**
  ...
  console.log('deleteAdjustBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listAdjustBalance = function(req, res, next) {
  /**
  ...
  console.log('listAdjustBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listAdjustBalance: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchAdjustBalance = function(req, res, next) {
  /**
   * Updates partially a AdjustBalance
   * This operation updates partially a AdjustBalance entity.
   *
   * id String Identifier of the AdjustBalance
   * adjustBalance AdjustBalance_Update The AdjustBalance to be updated
   * returns AdjustBalance
   **/

  console.log('patchAdjustBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchAdjustBalance',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchAdjustBalance error=" + error);
...
exports.retrieveAdjustBalance = function(req, res, next) {
  /**
  ...
  console.log('retrieveAdjustBalance :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveAdjustBalance: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
