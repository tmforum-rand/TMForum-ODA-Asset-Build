...
exports.createQuote = function(req, res, next) {
   ...
   *
   * quote Quote_Create The Quote to be created
   * returns Quote
   **/

  console.log('createQuote :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument quote */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createQuote', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createQuote', payload))
    .then(payload => {
    ...
    .catch( error => {
      console.log("createQuote: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteQuote = function(req, res, next) {
  ...
  console.log('deleteQuote :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listQuote = function(req, res, next) {
  /**
  ...
  console.log('listQuote :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listQuote: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchQuote = function(req, res, next) {
  /**
   * Updates partially a Quote
   * This operation updates partially a Quote entity.
   *
   * id String Identifier of the Quote
   * quote Quote_Update The Quote to be updated
   * returns Quote
   **/

  console.log('patchQuote :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchQuote',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchQuote error=" + error);
...
exports.retrieveQuote = function(req, res, next) {
  /**
  ...
  console.log('retrieveQuote :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveQuote: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
