...
exports.createTestSuite = function(req, res, next) {
  /**
   * Creates a TestSuite
   * This operation creates a TestSuite entity.
   *
   * testSuite TestSuite_Create The TestSuite to be created
   * returns TestSuite
   **/

  console.log('createTestSuite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument testSuite */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createTestSuite', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createTestSuite', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createTestSuite: error=" + error);
    ...
    .catch( error => {
      console.log("createTestSuite: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteTestSuite = function(req, res, next) {
  /**
   * Deletes a TestSuite
  ...
  console.log('deleteTestSuite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listTestSuite = function(req, res, next) {
  /**
  ...
  console.log('listTestSuite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listTestSuite: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchTestSuite = function(req, res, next) {
  /**
   * Updates partially a TestSuite
   * This operation updates partially a TestSuite entity.
   *
   * id String Identifier of the TestSuite
   * testSuite TestSuite_Update The TestSuite to be updated
   * returns TestSuite
   **/

  console.log('patchTestSuite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchTestSuite',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchTestSuite error=" + error);
...
exports.retrieveTestSuite = function(req, res, next) {
  /**
  ...
  console.log('retrieveTestSuite :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveTestSuite: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
