...
exports.createCustomer = function(req, res, next) {
   ...
   *
   * customer Customer_Create The Customer to be created
   * returns Customer
   **/

  console.log('createCustomer :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument customer */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createCustomer', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createCustomer', payload))
    .then(payload => {
    ...
    .catch( error => {
      console.log("createCustomer: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteCustomer = function(req, res, next) {
  ...
  console.log('deleteCustomer :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listCustomer = function(req, res, next) {
  ...
  console.log('listCustomer :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.patchCustomer = function(req, res, next) {
  /**
   * Updates partially a Customer
   * This operation updates partially a Customer entity.
   *
   * id String Identifier of the Customer
   * customer Customer_Update The Customer to be updated
   * returns Customer
   **/

  console.log('patchCustomer :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchCustomer',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchCustomer error=" + error);
...
exports.retrieveCustomer = function(req, res, next) {
  ...
  console.log('retrieveCustomer :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveCustomer: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
