...
exports.createServiceProblem = function(req, res, next) {
  /**
   * Creates a ServiceProblem
   * This operation creates a ServiceProblem entity.
   *
   * serviceProblem ServiceProblem_Create The ServiceProblem to be created
   * returns ServiceProblem
   **/

  console.log('createServiceProblem :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument serviceProblem */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createServiceProblem', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createServiceProblem', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createServiceProblem: error=" + error);
    ...
    .catch( error => {
      console.log("createServiceProblem: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteServiceProblem = function(req, res, next) {
  /**
  ...
  console.log('deleteServiceProblem :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listServiceProblem = function(req, res, next) {
  /**
  ...
  console.log('listServiceProblem :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listServiceProblem: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchServiceProblem = function(req, res, next) {
  /**
   * Updates partially a ServiceProblem
   * This operation updates partially a ServiceProblem entity.
   *
   * id String Identifier of the ServiceProblem
   * serviceProblem ServiceProblem_Update The ServiceProblem to be updated
   * returns ServiceProblem
   **/

  console.log('patchServiceProblem :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchServiceProblem',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchServiceProblem error=" + error);
...
exports.retrieveServiceProblem = function(req, res, next) {
  /**
  ...
  console.log('retrieveServiceProblem :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveServiceProblem: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
