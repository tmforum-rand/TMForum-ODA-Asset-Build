...
exports.createUnGroupAlarms = function(req, res, next) {
  /**
   * Creates a UnGroupAlarms
   * This operation creates a UnGroupAlarms entity.
   *
   * unGroupAlarms UnGroupAlarms_Create The UnGroupAlarms to be created
   * returns UnGroupAlarms
   **/

  console.log('createUnGroupAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument unGroupAlarms */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createUnGroupAlarms', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createUnGroupAlarms', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createUnGroupAlarms: error=" + error);
    ...
    .catch( error => {
      console.log("createUnGroupAlarms: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listUnGroupAlarms = function(req, res, next) {
  /**
  ...
  console.log('listUnGroupAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listUnGroupAlarms: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveUnGroupAlarms = function(req, res, next) {
  /**
  ...
  console.log('retrieveUnGroupAlarms :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveUnGroupAlarms: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
