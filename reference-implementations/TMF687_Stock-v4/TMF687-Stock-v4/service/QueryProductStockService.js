...
exports.createQueryProductStock = function(req, res, next) {
  /**
   * Creates a QueryProductStock
   * This operation creates a QueryProductStock entity.
   *
   * queryProductStock QueryProductStock_Create The QueryProductStock to be created
   * returns QueryProductStock
   **/

  console.log('createQueryProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument queryProductStock */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createQueryProductStock', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createQueryProductStock', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createQueryProductStock: error=" + error);
    ...
    .catch( error => {
      console.log("createQueryProductStock: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listQueryProductStock = function(req, res, next) {
  /**
  ...
  console.log('listQueryProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listQueryProductStock: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveQueryProductStock = function(req, res, next) {
  /**
  ...
  console.log('retrieveQueryProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveQueryProductStock: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
