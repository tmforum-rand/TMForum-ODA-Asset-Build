...
exports.createCheckProductStock = function(req, res, next) {
  /**
   * Creates a CheckProductStock
   * This operation creates a CheckProductStock entity.
   *
   * checkProductStock CheckProductStock_Create The CheckProductStock to be created
   * returns CheckProductStock
   **/

  console.log('createCheckProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument checkProductStock */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createCheckProductStock', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createCheckProductStock', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createCheckProductStock: error=" + error);
    ...
    .catch( error => {
      console.log("createCheckProductStock: error=" + error.toString());
      sendError(res, error);
    });
...
exports.listCheckProductStock = function(req, res, next) {
  /**
  ...
  console.log('listCheckProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listCheckProductStock: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.retrieveCheckProductStock = function(req, res, next) {
  /**
  ...
  console.log('retrieveCheckProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveCheckProductStock: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
