...
exports.createProductStock = function(req, res, next) {
  /**
   * Creates a ProductStock
   * This operation creates a ProductStock entity.
   *
   * productStock ProductStock_Create The ProductStock to be created
   * returns ProductStock
   **/

  console.log('createProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument productStock */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createProductStock', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createProductStock', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createProductStock: error=" + error);
    ...
    .catch( error => {
      console.log("createProductStock: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteProductStock = function(req, res, next) {
  /**
  ...
  console.log('deleteProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listProductStock = function(req, res, next) {
  /**
  ...
  console.log('listProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listProductStock: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchProductStock = function(req, res, next) {
  /**
   * Updates partially a ProductStock
   * This operation updates partially a ProductStock entity.
   *
   * id String Identifier of the ProductStock
   * productStock ProductStock_Update The ProductStock to be updated
   * returns ProductStock
   **/

  console.log('patchProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchProductStock',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchProductStock error=" + error);
...
exports.retrieveProductStock = function(req, res, next) {
  /**
  ...
  console.log('retrieveProductStock :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveProductStock: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
