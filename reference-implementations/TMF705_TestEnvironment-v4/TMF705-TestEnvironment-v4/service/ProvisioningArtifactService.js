...
exports.createProvisioningArtifact = function(req, res, next) {
  /**
   * Creates a ProvisioningArtifact
   * This operation creates a ProvisioningArtifact entity.
   *
   * provisioningArtifact ProvisioningArtifact_Create The ProvisioningArtifact to be created
   * returns ProvisioningArtifact
   **/

  console.log('createProvisioningArtifact :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument provisioningArtifact */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createProvisioningArtifact', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createProvisioningArtifact', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createProvisioningArtifact: error=" + error);
    ...
    .catch( error => {
      console.log("createProvisioningArtifact: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteProvisioningArtifact = function(req, res, next) {
  /**
  ...
  console.log('deleteProvisioningArtifact :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listProvisioningArtifact = function(req, res, next) {
  /**
  ...
  console.log('listProvisioningArtifact :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listProvisioningArtifact: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchProvisioningArtifact = function(req, res, next) {
  /**
   * Updates partially a ProvisioningArtifact
   * This operation updates partially a ProvisioningArtifact entity.
   *
   * id String Identifier of the ProvisioningArtifact
   * provisioningArtifact ProvisioningArtifact_Update The ProvisioningArtifact to be updated
   * returns ProvisioningArtifact
   **/

  console.log('patchProvisioningArtifact :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchProvisioningArtifact',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchProvisioningArtifact error=" + error);
...
exports.retrieveProvisioningArtifact = function(req, res, next) {
  /**
  ...
  console.log('retrieveProvisioningArtifact :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveProvisioningArtifact: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
