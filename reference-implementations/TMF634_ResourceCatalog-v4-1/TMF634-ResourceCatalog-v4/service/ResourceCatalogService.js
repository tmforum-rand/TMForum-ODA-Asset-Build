...
exports.createResourceCatalog = function(req, res, next) {
  /**
   * Creates a ResourceCatalog
   * This operation creates a ResourceCatalog entity.
   *
   * resourceCatalog ResourceCatalog_Create The ResourceCatalog to be created
   * returns ResourceCatalog
   **/

  console.log('createResourceCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument resourceCatalog */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createResourceCatalog', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createResourceCatalog', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createResourceCatalog: error=" + error);
    ...
    .catch( error => {
      console.log("createResourceCatalog: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteResourceCatalog = function(req, res, next) {
  /**
  ...
  console.log('deleteResourceCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listResourceCatalog = function(req, res, next) {
  /**
  ...
  console.log('listResourceCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listResourceCatalog: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchResourceCatalog = function(req, res, next) {
  /**
   * Updates partially a ResourceCatalog
   * This operation updates partially a ResourceCatalog entity.
   *
   * id String Identifier of the ResourceCatalog
   * resourceCatalog ResourceCatalog_Update The ResourceCatalog to be updated
   * returns ResourceCatalog
   **/

  console.log('patchResourceCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchResourceCatalog',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchResourceCatalog error=" + error);
...
exports.retrieveResourceCatalog = function(req, res, next) {
  /**
  ...
  console.log('retrieveResourceCatalog :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveResourceCatalog: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
