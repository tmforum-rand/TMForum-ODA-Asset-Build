...
exports.createCommunicationMessage = function(req, res, next) {
  /**
   * Creates a CommunicationMessage
   * This operation creates a CommunicationMessage entity.
   *
   * communicationMessage CommunicationMessage_Create The CommunicationMessage to be created
   * returns CommunicationMessage
   **/

  console.log('createCommunicationMessage :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);

  /* matching isRestfulCreate - argument communicationMessage */
  
  const resourceType = getResponseType(req);
  const requestSchema = getPayloadSchema(req);

  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req, 'createCommunicationMessage', payload))
    .then(payload => traverse(req, requestSchema, payload,[],getPayloadType(req)))
    .then(payload => processCommonAttributes(req, resourceType, payload))
    .then(payload => processAssignmentRules('createCommunicationMessage', payload))
    .then(payload => {
      ...
      mongoUtils.connect().then(db => {
        db.collection(resourceType)
          ...
          .catch((error) => {
            console.log("createCommunicationMessage: error=" + error);
    ...
    .catch( error => {
      console.log("createCommunicationMessage: error=" + error.toString());
      sendError(res, error);
    });
...
exports.deleteCommunicationMessage = function(req, res, next) {
  /**
  ...
  console.log('deleteCommunicationMessage :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
...
exports.listCommunicationMessage = function(req, res, next) {
  /**
  ...
  console.log('listCommunicationMessage :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  .then(db => {
    ...
    .then(stats => {
      ...
      .catch(error => {
        console.log("listCommunicationMessage: error=" + error);
        sendError(res, internalError);
      })
    })
    .catch(error => {
...
exports.patchCommunicationMessage = function(req, res, next) {
  /**
   * Updates partially a CommunicationMessage
   * This operation updates partially a CommunicationMessage entity.
   *
   * id String Identifier of the CommunicationMessage
   * communicationMessage CommunicationMessage_Update The CommunicationMessage to be updated
   * returns CommunicationMessage
   **/

  console.log('patchCommunicationMessage :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  swaggerUtils.getPayload(req)
    .then(payload => validateRequest(req,'patchCommunicationMessage',payload))
    .then(payload => traverse(req,requestSchema,payload,[],getPayloadType(req)))
    .then(payload => {
          ...
          db.collection(resourceType)
            ...
            .then(() => {
              db.collection(resourceType).findOne(query)
                ...
                .catch((error) => {
                  console.log("patchCommunicationMessage error=" + error);
...
exports.retrieveCommunicationMessage = function(req, res, next) {
  /**
  ...
  console.log('retrieveCommunicationMessage :: ' + req.method + ' ' + req.url + ' ' + req.headers.host);
  ...
  mongoUtils.connect().then(db => {
    db.collection(resourceType)
      ...
      .catch(error => {
        console.log("retrieveCommunicationMessage: error=" + error);
        sendError(res, internalError);
      });
  })
  .catch(error => {
