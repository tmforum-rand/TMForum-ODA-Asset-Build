...
var pmCollection = require('./CTK-Service_Inventory-4.0.0.postman_collection.json');
...
Object.keys(config['payloads']).forEach(resource => {
    var v = new Validator();
    var schema = require('./schemas/Service.schema.json');
...
function exportEnvironment(url) {
    ...
    envJson.values.forEach(element => {
        if (element.key == "Service_Inventory") {
