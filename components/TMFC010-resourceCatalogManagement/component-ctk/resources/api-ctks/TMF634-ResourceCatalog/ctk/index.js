...
var pmCollection = require('./TMF634-ResourceCatalog-v4.1.0.testkit.json');
...
Object.keys(config['payloads']).forEach(resource => {
    ...
    var schema = require('./schemas/ResourceSpecificationPatch.schema.json')
    valid = v.validate(config['payloads'][resource]['PATCH']['payload'], schema)
    ...
    if (resource == "ResourceSpecification") {
        pmCollection['item'][0]['item'][0]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['POST']['payload'])
        pmCollection['item'][1]['item'][10]['request']['body']['raw'] = JSON.stringify(config['payloads'][resource]['PATCH']['payload'])
    }
...
function exportEnvironment(url) {
    ...
    envJson.values.forEach(element => {
        if (element.key == "Resource_Catalog") {
